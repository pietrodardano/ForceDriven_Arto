...
def fplotterZ(forces, dur, columns):
    ...
    for fZ in forces:
        ...
        #filecsv = f"/home/user/arto_ws/src/RoboDataML/FrameLeverTop/Butt_{fZ}N_{dur}.csv"
        filecsv = f"/home/user/arto_ws/src/RoboDataML/Lever3StateUp/EmptyAriaRimontato_{fZ}N_{dur}.csv"
        
        df = pd.read_csv(filecsv, usecols=columns)
        file_name = filecsv.split("/")[-1].split(".")[0]
        w = df[columns[8]].to_numpy()
        af = MAF(w)

        plt.subplot(len(forces),1,c)
        plt.plot(df[columns[0]], df[columns[8]], label=labels[2])
...
def main():
    ...
    forces = [7,8,14]
    dur = [800, 1000, 1200]
    fplotterZ(forces, 2000, columns3)
