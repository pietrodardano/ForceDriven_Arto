...
data_folder = '/home/user/thesis_ws/src/ML_Levers_Knobs/DATA/1D_KNOB_Tz_ScalNorm/'
...
def preprocess_data(csv_path):
    try:
        ...
        if 'Torque_Z' in df.columns and 'Y' in df.columns:
            Tz = df["Torque_Z"]
            flag = 0
            filtered = myfilter(Tz, cutoff_freq=30)
            target_length = 2000
            if len(Tz) < target_length:
                padding_length = target_length - len(Tz)
                last_value = Tz.iloc[-1]
                # Pad the signal
                padded_signal = np.pad(Tz, (0, padding_length), mode='constant', constant_values=last_value)
                flag = 1
                noise_mean = 0
                noise_std = np.std(Tz-filtered)
                noise = np.random.normal(noise_mean, noise_std, padding_length)
                padded_signal[-padding_length:] += noise
            elif len(Tz)>target_length:
                padded_signal = Tz[-target_length:]
            else:
                padded_signal = Tz
                
            # Apply filtering
            filt_signal = myfilter(padded_signal, cutoff_freq=30)
            print(f"Original: {len(Tz)}, Padded: {len(filt_signal)}, FLAG={flag}")
