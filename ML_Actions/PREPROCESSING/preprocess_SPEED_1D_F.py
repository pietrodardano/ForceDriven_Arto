...
data_folder = '/home/user/thesis_ws/src/ML_ACTIONS/DATA/1D_SPEED_Fy_ScalNorm/'
folder_path = "/home/user/thesis_ws/src/ROBOT_ACTIONS_DATA/SPEED/"

def preprocess_data(csv_path):
    try:
        df = pd.read_csv(csv_path)
        if 'Force_Y' in df.columns and 'Y' in df.columns:
            Fx = df["Force_Y"]

            flag = 0
            filt_Fx = myfilter(Fx, cutoff_freq=30)
            target_length = 1800
            ...
            print(f"Skipping file {csv_path}: Columns 'Force_Y' or 'Y' are missing.")
